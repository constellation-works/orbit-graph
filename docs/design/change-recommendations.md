...
`.orbit-graph/change-history.1.sqlite3`. It is not task authority. External
...
## Public JSON import contract: version 1
...
- Delivery and task `source` plus capture time are stored independently. Empty
  identifiers, source systems, and capture times are rejected. Producers should
  use RFC 3339 timestamps. Git sync uses an explicit `unix:<seconds>` commit-time
  representation because Git itself does not supply a capture event.

The Rust contract is exposed by `DeliveryImport`, `DeliveryEvidence`,
`Provenance`, and `TaskAssociation`. Extracted output is exposed by
...
lines are mapped to byte ranges, then to the smallest intersecting named
tree-sitter symbol. A changed child line implicates the child, not its parent or
descendants. A delivery may span multiple commits; extraction compares its two
...
The v1 SQLite schema stores scope cursors, delivery payloads, normalized task
memberships, file stats/fallbacks, and both sides of symbol evidence. Foreign
keys cascade within a delivery. Repository, branch, schema version, extractor
...
## Known constraints and later evaluation
...
not emitted as a delivery in v1.

Stage 2 ranking must weight verified delivery evidence above Git-only evidence,
retain multi-task uncertainty, filter current-resolution results to `live`, and
measure file versus symbol precision separately. Stage 3 evaluation should use
