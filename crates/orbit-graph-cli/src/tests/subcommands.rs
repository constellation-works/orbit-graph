...
use crate::command::{Cli, CommandContext};

/// Recovered subcommand coverage running against an explicit fixture context.
#[test]
fn query_and_admin_subcommands_emit_json() {
    ...
    let graph_dir = worktree.path().join(".orbit-graph");
...
fn invalid_selector_errors_are_selector_parse_errors() {
    ...
    let cli =
        Cli::try_parse_from(["orbit-graph", "show", "not-a-selector"]).expect("parse show command");
    let error = cli
        .command
...
fn run_json(context: &CommandContext, args: &[&str]) -> Value {
    let cli = Cli::try_parse_from(std::iter::once("orbit-graph").chain(args.iter().copied()))
