...
#[doc(hidden)]
pub fn parse_timestamp(field: &str, timestamp: &str) -> Result<Timestamp, ExtractError> {
...
fn validate_timestamp(field: &str, timestamp: &str) -> Result<(), ExtractError> {
...
pub(crate) fn resolve_commit(repo: &Repository, revision: &str) -> Result<Oid, ExtractError> {
    let oid = Oid::from_str(revision).map_err(|error| {
        ExtractError::invalid_data("validate immutable Git revision", error.to_string())
    })?;
...
#[doc(hidden)]
pub fn branch_tip(repo: &Repository, branch: &str) -> Result<Oid, ExtractError> {
...
pub(crate) fn validate_delivery(
    ...
    if before == after
        || !repo.graph_descendant_of(after, before).map_err(|error| {
            ExtractError::invalid_data("validate delivery revision ancestry", error.to_string())
        })?
    {
        return Err(ExtractError::invalid_data(
    ...
    if tip != after
        && !repo.graph_descendant_of(tip, after).map_err(|error| {
            ExtractError::invalid_data("validate delivery landing branch", error.to_string())
        })?
    {
        return Err(ExtractError::invalid_data(
...
#[doc(hidden)]
pub fn validate_task_association(task: &TaskAssociation) -> Result<(), ExtractError> {
...
fn validate_temporal_fact(field: &str, fact: &TemporalFact) -> Result<(), ExtractError> {
    ...
    match (fact.status, fact.timestamp.as_deref()) {
        ...
        (TemporalStatus::Known, None) => Err(ExtractError::invalid_data(
        ...
        (TemporalStatus::Unavailable, Some(_)) => Err(ExtractError::invalid_data(
            "validate temporal fact",
...
fn invalid_timestamp(field: &str, timestamp: &str) -> ExtractError {
    ExtractError::invalid_data(
        "validate timestamp",
...
pub fn extract_delivery(
    repo: &Repository,
    delivery: DeliveryImport,
) -> Result<DeliveredChange, ExtractError> {
...
fn extract_tree_diff(
    ...
    before: Oid,
    after: Oid,
) -> Result<Vec<FileChange>, ExtractError> {
    ...
    for index in 0..diff.deltas().len() {
        let delta = diff.get_delta(index).ok_or_else(|| {
            ExtractError::invalid_data("read delivery diff", format!("missing delta {index}"))
        })?;
...
fn git_error(operation: &'static str) -> impl FnOnce(git2::Error) -> ExtractError {
    move |source| ExtractError::Git { operation, source }
}
