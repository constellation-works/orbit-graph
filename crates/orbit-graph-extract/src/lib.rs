// A library prints nothing (STD-02 §R15).
#![deny(clippy::print_stderr, clippy::print_stdout)]
#![cfg_attr(test, allow(clippy::expect_used, clippy::unwrap_used))]

//! Pure extraction contracts for the Orbit graph backend.
//!
//! This crate owns the language-neutral `ExtractedFile` shape, the trait
//! implemented by language-specific tree-sitter extractors, and the Git-tree
//! change extraction behind the delivery-history index. It intentionally
//! avoids storage, sync, query, and filesystem traversal concerns; the
//! `orbit-graph` library builds the index on top of it.

use std::path::Path;

mod error;
/// Raw extraction rows emitted for one source file.
pub mod extracted;
// Items marked `#[doc(hidden)]` in `history` and `languages` are public only
// for `orbit-graph`, which validates, stores and replays history with them;
// they are not part of the documented extraction contract.
pub mod history;
/// Language-specific extractor implementations.
pub mod languages;
/// Stable selector parser shared by graph callers and extractors.
pub mod selector;

pub use error::ExtractError;
pub use extracted::{
    ExtractedFile, RawCommand, RawConfig, RawImport, RawRef, RawRelation, RawString, RawSymbol,
};
pub use selector::{Selector, SelectorParseError};

#[cfg(test)]
mod tests;

/// Extracts graph rows from a single file's bytes.
pub trait Extractor {
    /// Returns the stable language identifier emitted for indexed files.
    fn lang(&self) -> &'static str;

    /// Returns whether this extractor should handle `path`.
    fn supports(&self, path: &Path) -> bool;

    /// Extracts raw graph rows from `bytes` at `path`.
    fn extract(&self, path: &Path, bytes: &[u8]) -> ExtractedFile;
}
