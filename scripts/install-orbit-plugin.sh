...
probe=$(printf '%s\n' '{"tool":"orbit.graph.version","input":{}}' |
    ORBIT_TOOL_NAME=orbit.graph.version "$graph_binary" 2>/dev/null) || {
        echo "incompatible orbit-graph binary: $graph_binary (expected v2 envelope, plugin_schema_version=1, extractor_version=17)" >&2
        exit 1
    }
case "$probe" in
    *'"ok":true'*'"extractor_version":17'*'"plugin_schema_version":1'*) ;;
    *) echo "incompatible orbit-graph binary: $graph_binary (expected v2 envelope, plugin_schema_version=1, extractor_version=17)" >&2; exit 1 ;;
esac
for manifest in \
