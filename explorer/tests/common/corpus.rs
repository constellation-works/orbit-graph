...
//!
//! `tests/fixtures/change-explorer/<case>/` holds a `base` tree, a `head` tree,
//! optionally a `root` tree, and an `expected.json` manifest. The root crate's
//! `tests/change_explorer_fixtures.rs` materializes those cases through the
//! `git` command line to verify the manifest against the `orbit-graph` binary.
...
/// `tests/change_explorer_fixtures.rs`.
